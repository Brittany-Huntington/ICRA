...
load("paper/eds_output_madejuly8.Rdata")
...
m<-read.csv("merged_PM_site_all_YR01.csv")
range(merged2025_eds_PM_S_colony1yr_sep$mean_PM)
range(site$prop_mean_PM)
range(m$mean_Sea_Surface_Temperature_jplMUR_Daily_YR01)
range(m$mean_biweekly_range_Sea_Surface_Temperature_jplMUR_Daily_YR01)
range(m$DHW.MeanMax_Degree_Heating_Weeks_CRW_Daily_YR01)
range(m$DHW.MeanDur_Major_Degree_Heating_Weeks_CRW_Daily_YR01)
...
range(site$mean_PM)
range(site$prop_mean_PM)
range(site$DHW_MeanMax)
range(site$DHW_Dur)
range(site$SST_Mean)
range(site$SST_biweekly)
...
plotResiduals(sim_res, site$DHW_MeanMax)
...
p<-ggplot() +
  ...
  xlab(expression(bold("Scaled SST mean ("*~degree*C*")"))) +
  scale_color_manual(values = colors) +
  scale_fill_manual(values = colors)
ggsave("paper/scaled_SST_mean_regression.png", plot = p, width = 8, height = 6, dpi = 300)

ggsave("paper/scaled_SST_mean_regression.png")
...
ggsave("paper/SST_mean_regression_scaled.png")
