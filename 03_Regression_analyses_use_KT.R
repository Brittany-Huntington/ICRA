...
#hist(site2$PER_DEAD_TRUE, breaks = 30)
...
# Format the data frame to match the structure you want
coeff_df_clean <- coeff_df %>%
  dplyr::select(Term = rownames(coeff_df), Estimate = Estimate, 
                Std_Error = `Std. Error`, z_value = `z value`, p_value = `Pr(>|z|)`) %>%
  mutate(
    p_value = case_when(
      p_value < 0.001 ~ "< 0.001",
      p_value < 0.01 ~ "< 0.01",
